    ...
    <div className="flex flex-col  mx-6 bg-white">
      <div className="flex flex-1 justify-between items-center mb-5">
        <div className="flex items-center">
          <img src={like} alt="Likes" className="w-6" /> <p className="ml-1 body3">{likes.length} Likes</p>
        </div>
      ...
      <div className="flex justify-around border-y border-accent4 p-1 md:p-2 m-1.5 mb-4">
        <button onClick={onClickLike} className="flex items-center">
          <LikeButton stroke={likes.includes(userId) ? colors.primary : colors.accent4} />
          <p className={`ml-1 ${likes.includes(userId) ? 'text-primary' : 'text-accent4'}  body3 font-medium`}>Like</p>
        </button>
        <button onClick={onClickCommentsButton} className="flex items-center">
          <CommentButton fill={isOpenedCommentsMenu ? colors.primary : colors.accent4} />
          <p className={`ml-1 ${isOpenedCommentsMenu ? 'text-primary' : 'text-accent4'} body3 font-medium`}>Comments</p>
        </button>
        <div className="flex items-center">
