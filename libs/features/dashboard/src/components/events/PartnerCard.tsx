...
const PartnerCard = ({ post }: PartnerCardPropsI) => {
  const { id, title, description, address, sports, author, labels, createdAt } =
    post;
    ...
    <div className="w-full max-w-2xl rounded-lg bg-white shadow-md px-3 pb-3 flex flex-col">
      <div className="flex justify-between px-4 py-3">
        <Link
          ...
          to={'/dashboard/' + SLUGS.UserProfile + '/' + author.id}
        >
          <div>
            <ProfileIcon />
          </div>
          <div className="flex flex-col self-center">
            ...
            <p className="text-xs text-main4 mb-1">{`${moment(createdAt).format(
              'DD MMMM hh:mm',
            )}`}</p>
          </div>
        </Link>
